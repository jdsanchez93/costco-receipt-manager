...
def normalize_item_name(name):
    """Normalize item names for comparison (uppercase, remove special chars, normalize whitespace)."""
    if not name:
        return ''
...
def match_item_by_name(items, discount_reference):
    """Find an item matching the discount reference by name (partial match supported)."""
    normalized_ref = normalize_item_name(discount_reference)
    if not normalized_ref:
        return None

    for item in reversed(items):
        normalized_item_name = normalize_item_name(item.get('item', ''))
        if normalized_ref in normalized_item_name or normalized_item_name in normalized_ref:
            return item
    return None
...
def parse_items_from_lines(lines):
    ...
    while i < len(lines) - 1:
        ...
        # Captures the item number and any text after / (could be numeric or item name)
        discount_match = re.match(r'^(?:E\s+)?(\d+)\s+/\s*(.+)$', line1)
        discount_price_match = re.match(r'^(-?\d+\.\d{2})-?\w*$', line2)

        if discount_match and discount_price_match:
            discount_reference = discount_match.group(2).strip()
            discount_amount = abs(float(discount_price_match.group(1)))
            matched_item = None

            # Strategy 1: Item number match (if reference is numeric)
            if discount_reference.isdigit():
                for item in reversed(items):
                    if item['item_number'] == discount_reference:
                        matched_item = item
                        break

            # Strategy 2: Name match
            if matched_item is None:
                matched_item = match_item_by_name(items, discount_reference)

            # Strategy 3: Positional fallback (most recent item)
            if matched_item is None and items:
                matched_item = items[-1]

            # Apply the discount
            if matched_item is not None:
                matched_item['discount'] += discount_amount
